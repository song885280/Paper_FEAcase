...
def Test(TypeName):
    ...
    """
    All_data = []
    vectors = np.load("G:\pythonProject\FEASimPaper\SBERT\FEA案例数据_英文400_sbert.npy")
    ...
    for i in tqdm(range(0, len(file_list))):
        for j in range(i + 1, len(file_list)):
            ...
            All_data.append(
                # src/ALL→src/Merged
                [case_1.strip("src/Merged"), case_2.strip("src/Merged"), sim_w2v, sim, sim_vsm, sim_BERT, sim_Human])
...
def Test_PSO(Path):
    ...
    for i in tqdm(range(0, len(file_list))):
        for j in range(i + 1, len(file_list)):
            ...
            sim_Human = SimCaculater(case_1, case_2)
            # src/ALL→src/Merged
            All_data.append([case_1.strip("src/Merged\\"), case_2.strip("src/Merged\\"), sim, sim_PSO, sim_vsm, sim_Human])
...
def TypeSide(TypeA, TypeB):
    ...
    for i in tqdm(range(0, len(file_listA))):
        for j in range(0, len(file_listB)):
            ...
            sim_vsm = VSM.VSIm(case_1, case_2)
            All_data.append([case_1.strip("src/Merged\\"), case_2.strip("src/Merged\\"), sim_w2v, simPairs, sim, sim_vsm])
